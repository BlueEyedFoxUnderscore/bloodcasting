package io.github.blueeyedfoxunderscore.bloodcasting.registry
...
typealias RegistrarEntry<T> = BloodcastingRegistrar<T>.Entry<out T>

abstract class BloodcastingRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Bloodcasting.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is BloodcastingRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
